...
def fill_header(val_do_dia, filename):
    val_do_dia.to_csv('dfs_2018/' + filename, header=['id_onibus','linha','linha_nome','undf','data_hora','undf2','tipo_validacao','sentido','integracao'], index=False)
    df = pd.read_csv('dfs_2018/' + filename)
    return df
...
def generate_df(filename):
    print(filename)
    val = pd.read_csv('./2018/' + filename, sep=';')
    ...
    validation_georeffed_do_dia['undf2'] = validation_georeffed_do_dia['undf2'].astype(np.int32)
...
if __name__ == '__main__':
    ...
    all_files = os.listdir('./2018/')

    for filename in all_files:
        generate_df(filename)

    final_df.to_csv('./data_input_nozerofill_2018.csv', sep=',', index=False)
